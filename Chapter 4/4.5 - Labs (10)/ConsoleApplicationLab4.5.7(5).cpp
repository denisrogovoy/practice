// ConsoleApplicationLab4.5.7(5).cpp: определяет точку входа для консольного приложения.
//
#include "stdafx.h"
#include <string>
#include <iostream>
...
int main()
{
	...
	int asteriks = 0, underline = 0, prevAsteriksPos = 0, prevUnderline = 0;
	string word = "";
	for (int i = 0; i < sentence.length(); i++)
	{
		if (sentence[i] == '*')
		{
			if (asteriks == 1)
			{
				...
				if (underline == 1)
				{
					sentence.replace(prevUnderline, 1, "&");
					underline = 0;
					word = "";
				}
				asteriks++;
		...
		if (sentence[i] == '_')
			if (underline == 1)
			{
				...
				for (int j = 0; j < word.length(); j++)
				{
					if (j == word.length() - 1)
						break;
					sentence.insert(prevUnderline + 1 + counter, " ");
					counter += 2;
				}
				i += word.length() - 1;
				sentence.replace(i, 1, " ");
